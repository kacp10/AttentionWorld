...
public class RegisterManager : MonoBehaviour
{
    ...
    private async Task HandleRegister()
    {
        ...
        Debug.Log($"Correo electrónico: {email}");
        Debug.Log($"Contraseña: {password}");
        Debug.Log($"Confirmar contraseña: {confirmPassword}");
        Debug.Log($"Año de nacimiento: {yearOfBirth}");
        Debug.Log($"Salón seleccionado: {selectedClassroom}");
        ...
        if (userRole == "Child")
        {
            ...
            Debug.Log("⚠️ Rol 'Child' seleccionado: No se guardará el correo.");
        }
        ...
        if (userRole == "Parents")
        {
            ...
            Debug.Log($"Padre seleccionado para el niño: {selectedChild}");
        }

        // Realizar la petición a DynamoDB
        try
        {
            await dynamoDBClient.PutItemAsync(request);
            feedbackText.text = "Registro exitoso. Por favor, inicia sesión.";
            feedbackText.color = Color.green;
            Debug.Log("✔ Registro exitoso en DynamoDB");
        ...
        {
            Debug.LogError($"❌ Error al registrar: {ex.Message}");
